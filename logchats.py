        ...
        if not no_log_pms_sql.is_approved(chat.id) and chat.id != 777000:
            if LOG_CHATS_.RECENT_USER != chat.id:
                ...
                LOG_CHATS_.NEWPM = await event.client.send_message(
                    ...
                    f"馃懁{_format.mentionuser(sender.first_name , sender.id)} has sent a new message \nId : `{chat.id}`",
                )
            try:
                if event.message:
...
async def log_tagged_messages(event):
    ...
    if full is not None:
        resalt += (
            f"\n<b>From : </b> 馃懁{_format.htmlmentionuser(full.first_name , full.id)}"
        )
...
CMD_HELP.update(
    {
        ...
        \n\n鈥�  **Syntax : **`.save`\
        \n鈥�  **Function : **__Saves tagged message in private group .__\
        \n\n鈥�  **Syntax : **`.log`\
        \n鈥�  **Function : **__By default will log all private chat messages if you use .nolog and want to log again then you need to use this__\
        \n\n鈥�  **Syntax : **`.nolog`\
        \n鈥�  **Function : **__Stops logging from a private chat or group where you used__\
        \n\n鈥�  **Syntax : **`.pmlog on/off`\
        \n鈥�  **Function : **__To turn on and turn off personal messages logging__\
        \n\n鈥�  **Syntax : **`.nolog`\
        \n鈥�  **Function : **__To turn on and turn off Group messages(tagged) logging__"
    }
)
