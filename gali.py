...
async def abusing(abused):
    reply_text = random.choice(marcusmemes.ABUSE_STRINGS)
...
async def fuckedd(abusehard):
    reply_text = random.choice(marcusmemes.ABUSEHARD_STRING)
...
async def metoo(e):
    txt = random.choice(marcusmemes.RENDISTR)
...
async def raping(raped):
    reply_text = random.choice(marcusmemes.RAPE_STRINGS)
...
async def chutiya(fuks):
    reply_text = random.choice(marcusmemes.CHU_STRINGS)
...
async def thanos(thanos):
    reply_text = random.choice(marcusmemes.THANOS_STRINGS)
...
async def _(event):
    if event.fwd_from:
        return
    marcusevent = await edit_or_reply(event, "`kiss`")
    animation_interval = 0.2
    animation_ttl = range(100)
    animation_chars = ["🤵       👰", "🤵     👰", "🤵  👰", "🤵💋👰"]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await marcusevent.edit(animation_chars[i % 4])
...
async def _(event):
    if event.fwd_from:
        return
    marcusevent = await edit_or_reply(event, "`fuking....`")
    animation_interval = 0.2
    animation_ttl = range(100)
...
async def _(event):
    if event.fwd_from:
        return
    marcusevent = await edit_or_reply(event, "`sex`")
    animation_interval = 0.2
    animation_ttl = range(100)
    animation_chars = ["🤵       👰", "🤵     👰", "🤵  👰", "🤵👼👰"]
    for i in animation_ttl:
