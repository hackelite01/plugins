...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Skull User"
...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await edit_or_reply(event, "Reply to any user message.馃槖馃")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await edit_or_reply(event, "Reply to media message馃槖馃")
        return
    chat = "@asciiart_bot"
    reply_message.sender
    if reply_message.sender.bot:
        await edit_or_reply(event, "Reply to actual users message.馃槖馃")
        return
    legendx22 = await edit_or_reply(event, "Wait making ASCII...馃馃敟馃敟")
