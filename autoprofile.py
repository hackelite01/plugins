...
DEFAULTUSERBIO = DEFAULT_BIO or " 釛棭釓嗎帰釓嗎憥釓� 釓炨弳釓︶棿 釒⑨弳釛搬棿  "
...
async def bloom_pfploop():
    ...
    while BLOOMSTART:
        ...
        drawn_text.text((95, 250), "      馃槇", font=ofnt, fill=(FR, FG, FB))
        img.save(autophoto_path)
        file = await bot.upload_file(autophoto_path)
        try:
            await bot(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        name = f"鈱氾笍 {HM}||鈥�  {DEFAULTUSER} 鈥箌|馃搮 {DM}"
        LOGS.info(name)
        try:
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        bio = f"馃搮 {DMY} | {DEFAULTUSERBIO} | 鈱氾笍 {HM}"
        LOGS.info(bio)
        try:
...
鈥�  **Syntax : **`.autopic angle`
鈥�  **Function : **__Rotating image along with the time on it with given angle if no angle is given then doesnt rotate. You need to set __`DEFAULT_PIC`__ in heroku__

鈥�  **Syntax : **`.digitalpfp`
鈥�  **Function : **__Your profile pic changes to digitaltime profile picutre__

鈥�  **Syntax : **`.bloom`
鈥�  **Function : **__Random colour profile pics will be set along with time on it. You need to set__ `DEFAULT_PIC`__ in heroku__

鈥�  **Syntax : **`.autoname`
鈥�  **Function : **__for time along with name, you must set __`AUTONAME`__ in the heroku vars first for this to work__

鈥�  **Syntax : **`.autobio`
鈥�  **Function : **__for time along with your bio, Set __`DEFAULT_BIO`__ in the heroku vars first__

鈥�  **Syntax : **`.end function`
鈥�  **Function : **__To stop the given functions like autopic ,difitalpfp , bloom , autoname and autobio__
