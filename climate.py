# SkullUserbot module for getting the weather of a city.
...
async def get_weather(weather):
    ...
    await edit_or_reply(
        weather,
        f"馃尅**Temperature:** `{celsius(curtemp)}掳C | {fahrenheit(curtemp)}掳F`\n"
        + f"馃グ**Human Feeling** `{celsius(feel)}掳C | {fahrenheit(feel)}掳F`\n"
        + f"馃ザ**Min. Temp.:** `{celsius(min_temp)}掳C | {fahrenheit(min_temp)}掳F`\n"
        + f"馃サ**Max. Temp.:** `{celsius(max_temp)}掳C | {fahrenheit(max_temp)}掳F`\n"
        + f"鈽侊笍**Humidity:** `{humidity}%`\n"
        + f"馃Ё**Pressure** `{pressure} hPa`\n"
        + f"馃尙**Wind:** `{kmph[0]} kmh | {mph[0]} mph, {findir}`\n"
        + f"鉀�**Cloud:** `{cloud} %`\n"
        + f"馃寗**Sunrise:** `{sun(sunrise)}`\n"
        + f"馃寘**Sunset:** `{sun(sunset)}`\n\n\n"
        + f"**{desc}**\n"
        + f"`{cityname}, {fullc_n}`\n"
        + f"`{time}`\n",
    )
...
CMD_HELP.update(
    {
        ...
        \n\n鈥�  **Syntax : **`.climate <city>`\
        \n鈥�  **Function : **__Gets the weather of a city. By default it is Delhi, change it by setcity__\
        \n\n鈥�  **Syntax : **`.setcity <city> or <country name/code>`\
        \n鈥�  **Function : **__Sets your default city so you can just use .weather or .climate.__\
        \n\n鈥�  **Syntax : **`.weather <city>`\
        \n鈥�  **Function : **__Gets the simple climate/weather information a city. By default it is Delhi, change it by setcity cmd__\
        \n\n鈥�  **Syntax : **`.wttr <city> `\
        \n鈥�  **Function : **__sends you the weather information for upcoming 3 days from today.__"
    }
)
