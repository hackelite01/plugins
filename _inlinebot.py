...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("**marcususerbot") and event.query.user_id == bot.uid:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        mayank = f"[{u.first_name}](tg://user?id={u.id})"
                except ValueError:
                    # ValueError: Could not find the input entity
                    mayank = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            buttons = [
                custom.Button.inline("show message 馃攼", data=f"secret_{timestamp}")
            ]
            result = builder.article(
                title="secret message",
                text=f"馃敀 A whisper message to {mayank}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += (
                "Use .unload {} to remove this plugin 漏marcususerbot".format(plugin_name)
            )
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
            except BaseException:
...
def paginate_help(page_number, loaded_plugins, prefix):
    ...
    if len(pairs) > number_of_rows:
        pairs = pairs[
            ...
            (
                custom.Button.inline(
                    "鈱�", data="{}_prev({})".format(prefix, modulo_page)
                ),
                custom.Button.inline("Close", data="close"),
                custom.Button.inline(
                    "鈱�", data="{}_next({})".format(prefix, modulo_page)
                ),
            )
        ]
    return pairs
