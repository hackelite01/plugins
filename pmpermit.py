...
USER_BOT_WARN_ZERO = "You were spamming my ultra legend pro master's inbox, henceforth you are blocked by my master's @marcususerbot. **Now GTFO, i'm watching webseries** "
...
if Config.PRIVATE_GROUP_ID != 0:
    ...
    async def block_p_m(event):
        ...
        if user.id in PM_START:
            PM_START.remove(user.id)
        await event.edit(
            f"`You are blocked Now .You Can't Message Me from now lol..`[{user.first_name}](tg://user?id={user.id})"
        )
    ...
    async def approve_p_m(event):
        ...
        if len(approved_users) > 0:
            for sender in approved_users:
                if sender.reason:
                    APPROVED_PMs += f"馃憠 [{sender.chat_id}](tg://user?id={sender.chat_id}) for {sender.reason}\n"
                else:
                    APPROVED_PMs += (
                        f"馃憠 [{sender.chat_id}](tg://user?id={sender.chat_id})\n"
                    )
        else:
...
CMD_HELP.update(
    {
        ...
        \n\n  鈥�  **Syntax : **`.approve or .a`\
        \n  鈥�  **Function : **__Approves the mentioned/replied person to PM.__\
        \n\n  鈥�  **Syntax : **`.disapprove or .da`\
        \n  鈥�  **Function : **__dispproves the mentioned/replied person to PM.__\
        \n\n  鈥�  **Syntax : **`.block`\
        \n  鈥�  **Function : **__Blocks the person.__\
        \n\n  鈥�  **Syntax : **`.unblock`\
        \n  鈥�  **Function : **__Unblocks the person.__\
        \n\n  鈥�  **Syntax : **`.listapproved`\
        \n  鈥�  **Function : **__To list the all approved users.__\
        \n\n  鈥�  **Syntax : **`.disapprove all or da all`\
        \n  鈥�  **Function : **__To disapprove all the approved users.__\
        \n\n  鈥�  Available variables for formatting `CUSTOM_PMPERMIT_TEXT` :\
